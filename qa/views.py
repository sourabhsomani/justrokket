...
from qa.models import (Answer, AnswerComment, AnswerVote, Question,
                       QuestionComment, QuestionVote, UserQAProfile, tag, BlogContent)
from taggit.models import Tag, TaggedItem
# from taggit.models import  Tag as DepTag, TaggedItem
...
class QuestionIndexView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['experts'] = showexperts()
        context['tags'] = showtags()
        ...
        context['blogcontent'] = list(BlogContent.objects.all().order_by('-id')[:2].values())
        
        return context

    def get_queryset(self):
        queryset = super(QuestionIndexView, self).get_queryset().filter(approve=True,answer__approve=True).select_related('user').annotate(num_answers=Count('answer', distinct=True),
                      num_question_comments=Count('questioncomment',
                      distinct=True))
        return queryset
...
class QuestionsByTagView(ListView):
    ...
    def get_queryset(self, **kwargs):
        ...
        return Question.objects.filter(tag__name=self.kwargs['tag'],approve=True,answer__approve=True)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['anscount'] = Answer.objects.count
        context['noans'] = Question.objects.order_by('-pub_date').filter(
            tag__name__contains=self.kwargs['tag'], answer__isnull=True)[:10]
        context['reward'] = Question.objects.order_by('-reward').filter(
            tag__name__contains=self.kwargs['tag'],
            reward__gte=1)[:10]
        context['totalnoans'] = len(context['noans'])
        context['blogcontent'] = BlogContent.objects.filter(tag__name=self.kwargs['tag']).order_by('-id')[:2].values()
        context['experts'] = showexperts()
        context['tags'] = showtags()
        return context
...
@csrf_exempt
def CreateAnswerFromProfilePage(request):
    ...
    if request.method=='POST':
        ...
        Answer.objects.create(question=question,answer_text=data['answer_text'],user=Profile.objects.get(user=request.user))


        data = json.dumps([{"message": "created"}]) 
...
class ParentVoteView(View):
    ...
    def post(self, request, object_id):
        ...
        if vote_target.user.user == request.user:
            raise ValidationError(
            ...
            vote, created = self.vote_model.objects.get_or_create(
                ...
                **object_kwargs)
            if created:
                vote_target.user.user.userqaprofile.points += 1 if upvote else -1
                if upvote:
                ...
                if vote.value == upvote:
                    ...
                    vote_target.user.user.userqaprofile.points += 2 if upvote else -2
                    vote.value = upvote
                    vote.save()
                    if upvote:
                        vote_target.positive_votes += 1
            ...
            vote_target.user.user.userqaprofile.save()
            if self.model == Question:
...
def profile(request, user_id, name):
    ...
    profile = Profile.objects.get(user=user_ob)

    tags = showtags()
    experts = showexperts()
    context = {'user': profile, 'tags':tags, 'experts':experts}
    print(profile)
...
class TagView(View):
     ...
     def get(self,request):
        ...
        tag_query = request.GET['q']

        result = tag.objects.filter(name__startswith=tag_query)
...
def showexperts():
    return Profile.objects.filter(~Q(user_type="A"))[:2]
